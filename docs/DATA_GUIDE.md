...
| **우량관측소** | `rainfall` | 강수량 측정 시설 | 춘천시(소양2교), 광주시(남한산초교) |
...
**주요 차이점:**
- `datetime` 필드로 날짜 제공 (댐 데이터의 `tm`과 다름)
- `values` 객체 내에 측정항목별로 중첩 구조
- 값과 함께 `unit`(단위), `rst_id`(결과 ID) 포함
- 월 단위(`mt_1`)만 제공
...
for item in result['data']:
    date = item['datetime']
    toc_value = item['values']['총유기탄소(TOC)']['value']
    unit = item['values']['총유기탄소(TOC)']['unit']
    print(f"{date}: {toc_value} {unit}")
# 출력: 2025-03-01 00:00: 1.4 mg/L
```
...
K-water 원본 API는 **조회 기간에 제한이 없습니다**. 30년 이상의 데이터도 1-2초 내에 조회 가능합니다.
실제 제한은 시설별로 **저장된 데이터의 범위**입니다.

#### 댐/수위/우량/기상 관측소

| 시간 단위 | 원본 API 권장 | SDK 제한 | 설명 |
|-----------|--------------|---------|------|
| `h_1` (시간) | **6개월** | 제한 없음 | 시간별 데이터 조회 |
| `d_1` (일) | **5년** | 제한 없음 | 일별 데이터 조회 (30년+ 조회 가능) |

#### 수질관측소

| 시간 단위 | 원본 API 권장 | SDK 제한 | 설명 |
|-----------|--------------|---------|------|
| `mt_1` (월) | - | 제한 없음 | 월별 데이터만 제공 (수질 측정 특성) |

> ℹ️ **참고**:
> - **원본 API 권장 기간**은 K-water의 공식 권장사항입니다
> - **실제 조회 가능 기간**은 저장된 데이터 범위에만 의존합니다
> - **10분 단위 (`min_10`)는 지원되지 않습니다** ❌
> - 긴 기간 조회 시 응답 시간: 10년 0.2초, 20년 1.2초, 30년 1.5초
...
**일별 데이터 (d_1)**
- ✅ **권장**: 5년 이내 (원본 API 권장)
- 💡 **참고**: 30년 이상도 조회 가능하나 원본 API 권장사항을 따르는 것이 좋습니다
- 📊 **용도**: 장기 추세 분석, 연간 비교
